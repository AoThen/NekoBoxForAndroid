package com.orbit.shuttle.group
...
import com.orbit.shuttle.fmt.hysteria.parseHysteria1Json
...
import com.orbit.shuttle.fmt.v2ray.isTLS
...
object RawUpdater : GroupUpdater() {
    ...
    override suspend fun doUpdate(
        ...
        val exists = SeeWDatabase.proxyDao.getByGroup(proxyGroup.id)
        ...
        for ((name, bean) in nameMap.entries) {
            if (toReplace.contains(name)) {
                ...
                changed++
                SeeWDatabase.proxyDao.addProxy(
                    ProxyEntity(
        ...
        SeeWDatabase.proxyDao.updateProxy(toUpdate).also {
            Logs.d("Updated profiles: $it")
        }

        SeeWDatabase.proxyDao.deleteProxy(toDelete).also {
            Logs.d("Deleted profiles: $it")
        }

        val existCount = SeeWDatabase.proxyDao.countByGroup(proxyGroup.id).toInt()
        ...
        SeeWDatabase.groupDao.updateGroup(proxyGroup)
