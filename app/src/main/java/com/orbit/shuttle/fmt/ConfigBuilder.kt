...
import com.orbit.shuttle.*
import com.orbit.shuttle.bg.VpnService
import com.orbit.shuttle.database.DataStore
import com.orbit.shuttle.database.ProxyEntity
import com.orbit.shuttle.database.ProxyEntity.Companion.TYPE_CONFIG
import com.orbit.shuttle.database.SeeWDatabase
import com.orbit.shuttle.fmt.ConfigBuildResult.IndexEntity
import com.orbit.shuttle.fmt.hysteria.HysteriaBean
import com.orbit.shuttle.fmt.hysteria.buildSingBoxOutboundHysteriaBean
import com.orbit.shuttle.fmt.internal.ChainBean
import com.orbit.shuttle.fmt.shadowsocks.ShadowsocksBean
import com.orbit.shuttle.fmt.shadowsocks.buildSingBoxOutboundShadowsocksBean
import com.orbit.shuttle.fmt.socks.SOCKSBean
import com.orbit.shuttle.fmt.socks.buildSingBoxOutboundSocksBean
import com.orbit.shuttle.fmt.ssh.SSHBean
import com.orbit.shuttle.fmt.ssh.buildSingBoxOutboundSSHBean
import com.orbit.shuttle.fmt.tuic.TuicBean
import com.orbit.shuttle.fmt.tuic.buildSingBoxOutboundTuicBean
import com.orbit.shuttle.fmt.v2ray.StandardV2RayBean
import com.orbit.shuttle.fmt.v2ray.buildSingBoxOutboundStandardV2RayBean
import com.orbit.shuttle.fmt.wireguard.WireGuardBean
import com.orbit.shuttle.fmt.wireguard.buildSingBoxOutboundWireguardBean
import com.orbit.shuttle.ktx.isIpAddress
import com.orbit.shuttle.ktx.mkPort
import com.orbit.shuttle.utils.PackageCache
import com.orbit.shuttle.module.*
import com.orbit.shuttle.module.SingBoxOptions.*
import com.orbit.shuttle.module.plugin.Plugins
import com.orbit.shuttle.module.proxy.anytls.AnyTLSBean
import com.orbit.shuttle.module.proxy.anytls.buildSingBoxOutboundAnyTLSBean
import com.orbit.shuttle.module.proxy.config.ConfigBean
import com.orbit.shuttle.module.proxy.shadowtls.ShadowTLSBean
import com.orbit.shuttle.module.proxy.shadowtls.buildSingBoxOutboundShadowTLSBean
import com.orbit.shuttle.module.utils.JavaUtil.gson
import com.orbit.shuttle.module.utils.Util
import com.orbit.shuttle.module.utils.listByLineOrComma
...
fun buildConfig(
    ...
    val group = SeeWDatabase.groupDao.getById(proxy.groupId)

    fun ProxyEntity.resolveChainInternal(): MutableList<ProxyEntity> {
        val bean = requireBean()
        if (bean is ChainBean) {
            val beans = SeeWDatabase.proxyDao.getEntities(bean.proxies)
    ...
    fun ProxyEntity.resolveChain(): MutableList<ProxyEntity> {
        val thisGroup = SeeWDatabase.groupDao.getById(groupId)
        val frontProxy = thisGroup?.frontProxy?.let { SeeWDatabase.proxyDao.getById(it) }
        val landingProxy = thisGroup?.landingProxy?.let { SeeWDatabase.proxyDao.getById(it) }
    ...
    val extraRules = if (forTest) listOf() else SeeWDatabase.rulesDao.enabledRules()
    val extraProxies =
        if (forTest) mapOf() else SeeWDatabase.proxyDao.getEntities(extraRules.mapNotNull { rule ->
    ...
    return MyOptions().apply {
            ...
            profileList.forEachIndexed { index, proxyEntity ->
                ...
                if (bean.canMapping() && proxyEntity.needExternal()) {
                    ...
                    if (index == profileList.lastIndex) {
                        ...
                        if (Plugins.isUsingNekoExe(pluginId)) {
                            needExternal = false
        ...
        // build outbounds
        if (buildSelector) {
            val list = group.id.let { SeeWDatabase.proxyDao.getByGroup(it) }
            list.forEach {
        ...
        for (rule in extraRules) {
            ...
            val uidList = rule.packages.map {
                if (!isVPN) {
                    Toast.makeText(
                        SeeWApp.application,
                        SeeWApp.application.getString(R.string.route_need_vpn, rule.displayName()),
                        Toast.LENGTH_SHORT
                    ).show()
                }
