...
import com.orbit.shuttle.GroupOrder
import com.orbit.shuttle.GroupType
...
import com.orbit.shuttle.bg.BaseService
import com.orbit.shuttle.bg.proto.UrlTest
import com.orbit.shuttle.database.DataStore
import com.orbit.shuttle.database.GroupManager
import com.orbit.shuttle.database.ProfileManager
import com.orbit.shuttle.database.ProxyEntity
import com.orbit.shuttle.database.ProxyGroup
...
import com.orbit.shuttle.databinding.LayoutProfileListBinding
import com.orbit.shuttle.databinding.LayoutProgressListBinding
import com.orbit.shuttle.fmt.AbstractBean
import com.orbit.shuttle.fmt.toUniversalLink
import com.orbit.shuttle.group.GroupUpdater
import com.orbit.shuttle.group.RawUpdater
import com.orbit.shuttle.ktx.FixedLinearLayoutManager
import com.orbit.shuttle.ktx.Logs
import com.orbit.shuttle.ktx.SubscriptionFoundException
import com.orbit.shuttle.ktx.alert
import com.orbit.shuttle.ktx.app
import com.orbit.shuttle.ktx.dp2px
import com.orbit.shuttle.ktx.getColorAttr
import com.orbit.shuttle.ktx.getColour
import com.orbit.shuttle.ktx.isIpAddress
...
import com.orbit.shuttle.ktx.runOnLifecycleDispatcher
import com.orbit.shuttle.ktx.runOnMainDispatcher
import com.orbit.shuttle.ktx.scrollTo
import com.orbit.shuttle.ktx.showAllowingStateLoss
import com.orbit.shuttle.ktx.snackbar
import com.orbit.shuttle.ktx.startFilesForResult
import com.orbit.shuttle.ktx.tryToShow
import com.orbit.shuttle.plugin.PluginManager
import com.orbit.shuttle.ui.profile.ChainSettingsActivity
import com.orbit.shuttle.ui.profile.HttpSettingsActivity
import com.orbit.shuttle.ui.profile.HysteriaSettingsActivity
import com.orbit.shuttle.ui.profile.MieruSettingsActivity
import com.orbit.shuttle.ui.profile.NaiveSettingsActivity
import com.orbit.shuttle.ui.profile.SSHSettingsActivity
import com.orbit.shuttle.ui.profile.ShadowsocksSettingsActivity
import com.orbit.shuttle.ui.profile.SocksSettingsActivity
import com.orbit.shuttle.ui.profile.TrojanGoSettingsActivity
import com.orbit.shuttle.ui.profile.TrojanSettingsActivity
import com.orbit.shuttle.ui.profile.TuicSettingsActivity
import com.orbit.shuttle.ui.profile.VMessSettingsActivity
import com.orbit.shuttle.ui.profile.WireGuardSettingsActivity
import com.orbit.shuttle.widget.QRCodeDialog
...
import com.orbit.shuttle.module.Protocols.getProtocolColor
import com.orbit.shuttle.module.proxy.anytls.AnyTLSSettingsActivity
import com.orbit.shuttle.module.proxy.config.ConfigSettingActivity
import com.orbit.shuttle.module.proxy.shadowtls.ShadowTLSSettingsActivity
import com.orbit.shuttle.module.ui.ConnectionTestNotification
...
class ConfigurationFragment @JvmOverloads constructor(
    ...
    override fun onMenuItemClick(item: MenuItem): Boolean {
        when (item.itemId) {
            ...
            R.id.action_import_clipboard -> {
                val text = SeeWApp.getClipboardText()
                if (text.isBlank()) {
            ...
            R.id.action_clear_traffic_statistics -> {
                runOnDefaultDispatcher {
                    val profiles = SeeWDatabase.proxyDao.getByGroup(DataStore.currentGroupId())
    ...
    fun pingTest(icmpPing: Boolean) {
        ...
        val mainJob = runOnDefaultDispatcher {
            val profilesList = SeeWDatabase.proxyDao.getByGroup(group.id).filter {
                if (icmpPing) {
                ...
                testJobs.add(launch(Dispatchers.IO) {
                    while (isActive) {
                        ...
                        if (!address.isIpAddress()) {
                            try {
                                SeeWApp.underlyingNetwork!!.getAllByName(address).apply {
                                    if (isNotEmpty()) {
                        ...
                        try {
                            if (icmpPing) {
                                ...
                                val socket =
                                    SeeWApp.underlyingNetwork?.socketFactory?.createSocket()
                                        ?: Socket()
                                try {
    ...
    fun urlTest() {
        ...
        val mainJob = runOnDefaultDispatcher {
            val profilesList = SeeWDatabase.proxyDao.getByGroup(group.id)
            test.proxyN = profilesList.size
            val profiles = ConcurrentLinkedQueue(profilesList)
            repeat(DataStore.connectionTestConcurrent) {
                testJobs.add(launch(Dispatchers.IO) {
    ...
    inner class GroupPagerAdapter : FragmentStateAdapter(this),
        ...
        fun reload(now: Boolean = false) {
            ...
            runOnDefaultDispatcher {
                var newGroupList = ArrayList(SeeWDatabase.groupDao.allGroups())
                if (newGroupList.isEmpty()) {
                    SeeWDatabase.groupDao.createGroup(ProxyGroup(ungrouped = true))
                    newGroupList = ArrayList(SeeWDatabase.groupDao.allGroups())
                }
                newGroupList.find { it.ungrouped }?.let {
                    if (SeeWDatabase.proxyDao.countByGroup(it.id) == 0L) {
        ...
        override suspend fun onRemoved(groupId: Long, profileId: Long) {
            ...
            if (group.ungrouped && SeeWDatabase.proxyDao.countByGroup(groupId) == 0L) {
                reload()
            }
        }
    }
    ...
    class GroupFragment : Fragment() {
        ...
        inner class ConfigurationAdapter : RecyclerView.Adapter<ConfigurationHolder>(),
            ...
            fun commitMove() = runOnDefaultDispatcher {
                updated.forEach { SeeWDatabase.proxyDao.updateProxy(it) }
                updated.clear()
            }
            ...
            override suspend fun groupUpdated(groupId: Long) {
                if (groupId != proxyGroup.id) return
                proxyGroup = SeeWDatabase.groupDao.getById(groupId)!!
                reloadProfiles()
            }

            fun reloadProfiles() {
                var newProfiles = SeeWDatabase.proxyDao.getByGroup(proxyGroup.id)
                when (proxyGroup.order) {
                    GroupOrder.BY_NAME -> {
        ...
        inner class ConfigurationHolder(val view: View) : RecyclerView.ViewHolder(view),
                ...
                if (select) {
                    ...
                    view.setOnClickListener {
                        runOnDefaultDispatcher {
                            ...
                            } else if (SeeWApp.isTv) {
                                if (DataStore.serviceState.started) {
                                    SeeWApp.stopService()
                                } else {
                                    SeeWApp.startService()
                                }
                            }
                        }
            ...
            fun export(link: String) {
                val success = SeeWApp.trySetPrimaryClip(link)
