...
import com.orbit.shuttle.bg.Executable
...
import com.orbit.shuttle.databinding.LayoutBackupBinding
import com.orbit.shuttle.databinding.LayoutImportBinding
import com.orbit.shuttle.databinding.LayoutProgressBinding
...
class BackupFragment : NamedFragment(R.layout.layout_backup) {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        binding.actionExport.setOnClickListener {
            runOnDefaultDispatcher {
                ...
                onMainDispatcher {
                    startFilesForResult(
                        exportSettings, "seew_backup_${Date().toLocaleString()}.json"
                    )
                }
            }
        }
        ...
        binding.actionShare.setOnClickListener {
            runOnDefaultDispatcher {
                ...
                app.cacheDir.mkdirs()
                val cacheFile = File(
                    app.cacheDir, "seew_backup_${Date().toLocaleString()}.json"
                )
        ...
        val out = JSONObject().apply {
            ...
            if (profile) {
                put("profiles", JSONArray().apply {
                    SeeWDatabase.proxyDao.getAll().forEach {
                        put(it.toBase64Str())
                    }
                })

                put("groups", JSONArray().apply {
                    SeeWDatabase.groupDao.allGroups().forEach {
                        put(it.toBase64Str())
                    }
                })
            }
            if (rule) {
                put("rules", JSONArray().apply {
                    SeeWDatabase.rulesDao.allRules().forEach {
                        put(it.toBase64Str())
                    }
                })
            }
    ...
    fun finishImport(
        ...
        if (profile && content.has("profiles")) {
            ...
            SeeWDatabase.proxyDao.reset()
            SeeWDatabase.proxyDao.insert(profiles)
            ...
            SeeWDatabase.groupDao.reset()
            SeeWDatabase.groupDao.insert(groups)
        }
        if (rule && content.has("rules")) {
            ...
            SeeWDatabase.rulesDao.reset()
            SeeWDatabase.rulesDao.insert(rules)
        }
