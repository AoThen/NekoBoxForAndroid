...
import com.orbit.shuttle.database.*
import com.orbit.shuttle.databinding.LayoutGroupItemBinding
...
import com.orbit.shuttle.module.utils.toBytesString
...
class GroupFragment : ToolbarFragment(R.layout.layout_group),
            ...
            R.id.action_update_all -> {
                MaterialAlertDialogBuilder(requireContext()).setTitle(R.string.confirm)
                    ...
                    .setPositiveButton(R.string.yes) { _, _ ->
                        SeeWDatabase.groupDao.allGroups()
        ...
        registerForActivityResult(ActivityResultContracts.CreateDocument()) { data ->
            if (data != null) {
                runOnDefaultDispatcher {
                    val profiles = SeeWDatabase.proxyDao.getByGroup(selectedGroup.id)
    ...
    inner class GroupAdapter : RecyclerView.Adapter<GroupHolder>(),
        ...
        suspend fun reload() {
            val groups = SeeWDatabase.groupDao.allGroups().toMutableList()
            if (groups.size > 1 && SeeWDatabase.proxyDao.countByGroup(groups.find { it.ungrouped }!!.id) == 0L) groups.removeAll { it.ungrouped }
            groupList.clear()
        ...
        fun commitMove() = runOnDefaultDispatcher {
            updated.forEach { SeeWDatabase.groupDao.updateGroup(it) }
            updated.clear()
        }
        ...
        override suspend fun groupRemoved(groupId: Long) {
            ...
            onMainDispatcher {
                undoManager.flush()
                if (SeeWDatabase.groupDao.allGroups().size <= 2) {
                    runOnDefaultDispatcher {
                        reload()
                    }
                } else {
    ...
    inner class GroupHolder(binding: LayoutGroupItemBinding) :
            ...
            when (item.itemId) {
                ...
                R.id.action_export_clipboard -> {
                    runOnDefaultDispatcher {
                        ...
                        val links = profiles.joinToString("\n") { it.toStdLink(compact = true) }
                        onMainDispatcher {
                            SeeWApp.trySetPrimaryClip(links)
        ...
        fun bind(group: ProxyGroup) {
            ...
            runOnDefaultDispatcher {
                val size = SeeWDatabase.proxyDao.countByGroup(group.id)
                onMainDispatcher {
