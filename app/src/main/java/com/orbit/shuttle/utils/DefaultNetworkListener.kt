...
object DefaultNetworkListener {
    ...
    suspend fun get() = if (fallback) @TargetApi(23) {
        SeeWApp.connectivity.activeNetwork
        ...
        try {
            ...
            when (Build.VERSION.SDK_INT) {
                in 31..Int.MAX_VALUE -> @TargetApi(31) {
                    SeeWApp.connectivity.registerBestMatchingNetworkCallback(
                        request, Callback, mainHandler
                    )
                }
                in 28 until 31 -> @TargetApi(28) {  // we want REQUEST here instead of LISTEN
                    SeeWApp.connectivity.requestNetwork(request, Callback, mainHandler)
                }
                in 26 until 28 -> @TargetApi(26) {
                    SeeWApp.connectivity.registerDefaultNetworkCallback(Callback, mainHandler)
                }
                in 24 until 26 -> @TargetApi(24) {
                    SeeWApp.connectivity.registerDefaultNetworkCallback(Callback)
                }
                else -> {
                    SeeWApp.connectivity.requestNetwork(request, Callback)
    ...
    private fun unregister() = SeeWApp.connectivity.unregisterNetworkCallback(Callback)
}
