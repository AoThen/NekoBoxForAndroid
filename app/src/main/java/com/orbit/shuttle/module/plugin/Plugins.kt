package com.orbit.shuttle.module.plugin
...
object Plugins {
    const val AUTHORITIES_PREFIX_SEKAI_EXE = "com.orbit.shuttle.plugin."
    const val AUTHORITIES_PREFIX_NEKO_EXE = "com.orbit.shuttle.exe."

    const val ACTION_NATIVE_PLUGIN = "com.orbit.shuttle.plugin.ACTION_NATIVE_PLUGIN"

    const val METADATA_KEY_ID = "com.orbit.shuttle.plugin.id"
    const val METADATA_KEY_EXECUTABLE_PATH = "com.orbit.shuttle.plugin.executable_path"
    ...
    fun isUsingNekoExe(pluginId: String): Boolean {
    ...
    fun getPluginExternal(pluginId: String): ProviderInfo? {
        ...
        if (providers.size > 1) {
            ...
            Toast.makeText(SeeWApp.application, message, Toast.LENGTH_LONG).show()
        }
    ...
    private fun getExtPluginOld(pluginId: String): List<ProviderInfo> {
        ...
        val list1 = SeeWApp.application.packageManager.queryIntentContentProviders(
            Intent(ACTION_NATIVE_PLUGIN, buildUri(pluginId, "com.orbit.shuttle")), flags
        )
        val list2 = SeeWApp.application.packageManager.queryIntentContentProviders(
            Intent(ACTION_NATIVE_PLUGIN, buildUri(pluginId, "com.orbit.shuttle")), flags
        )
