...
import com.orbit.shuttle.BootReceiver
...
import com.orbit.shuttle.bg.proto.ProxyInstance
...
import com.orbit.shuttle.utils.DefaultNetworkListener
...
class BaseService {
            ...
            when (intent.action) {
                ...
                PowerManager.ACTION_DEVICE_IDLE_MODE_CHANGED -> {
                    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
                        if (SeeWApp.power.isDeviceIdleMode) {
                            proxy?.box?.sleep()
                        } else {
                            proxy?.box?.wake()
    ...
    class Binder(private var data: Data? = null) : ISeeWService.Stub(), CoroutineScope,
        AutoCloseable {
        private val callbacks = object : RemoteCallbackList<ISeeWServiceCallback>() {
            override fun onCallbackDied(callback: ISeeWServiceCallback?, cookie: Any?) {
                super.onCallbackDied(callback, cookie)
            }
        }

        val callbackIdMap = mutableMapOf<ISeeWServiceCallback, Int>()
        ...
        override fun registerCallback(cb: ISeeWServiceCallback, id: Int) {
            if (id == SeeWConnection.CONNECTION_ID_RESTART_BG) {
        ...
        suspend fun broadcast(work: (ISeeWServiceCallback) -> Unit) {
        ...
        override fun unregisterCallback(cb: ISeeWServiceCallback) {
    ...
    interface Interface {
        ...
        fun reload() {
            ...
            if (canReloadSelector()) {
                val ent = SeeWDatabase.proxyDao.getById(DataStore.selectedProxy)
        ...
        fun canReloadSelector(): Boolean {
            ...
            val ent = SeeWDatabase.proxyDao.getById(DataStore.selectedProxy) ?: return false
        ...
        suspend fun preInit() {
            DefaultNetworkListener.start(this) {
                SeeWApp.connectivity.getLinkProperties(it)?.also { link ->
                    SeeWApp.underlyingNetwork = it
        ...
        fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
            ...
            val profile = SeeWDatabase.proxyDao.getById(DataStore.selectedProxy)
            this as Context
