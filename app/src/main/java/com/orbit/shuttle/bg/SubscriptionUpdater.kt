...
object SubscriptionUpdater {
    ...
    private const val WORK_NAME = "SubUpdate"

    suspend fun reconfigureUpdater() {
        RemoteWorkManager.getInstance(app).cancelUniqueWork(WORK_NAME)

        val subscriptions = SeeWDatabase.groupDao.subscriptions()
    ...
    class UpdateTask(
        ...
        val notification = NotificationCompat.Builder(applicationContext, "ch_sub")
            .setWhen(0)
        ...
        override suspend fun doWork(): Result {
            var subscriptions =
                SeeWDatabase.groupDao.subscriptions().filter { it.subscription!!.autoUpdate }
