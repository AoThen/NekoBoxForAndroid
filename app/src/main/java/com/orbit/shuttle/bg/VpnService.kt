...
import com.orbit.shuttle.fmt.LOCALHOST
import com.orbit.shuttle.fmt.hysteria.HysteriaBean
import com.orbit.shuttle.ktx.*
import com.orbit.shuttle.ui.VpnRequestActivity
import com.orbit.shuttle.utils.Subnet
...
class VpnService : BaseVpnService(),
    ...
    override fun acquireWakeLock() {
        wakeLock = SeeWApp.power.newWakeLock(PowerManager.PARTIAL_WAKE_LOCK, "seew:vpn")
            .apply { acquire() }
    }
    ...
    override val tag = "SeeWVpn"
    override fun createNotification(profileName: String) =
        ServiceNotification(this, profileName, "ch_vpn")
...
//        val tunOptions = JSONObject(tunOptionsJson)
        ...
        // address & route & MTU ...... use SeeW GUI config
        val builder = Builder().setConfigureIntent(SeeWApp.configureIntent(this))
    ...
    fun updateUnderlyingNetwork(builder: Builder? = null) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP_MR1) {
            SeeWApp.underlyingNetwork?.let {
                builder?.setUnderlyingNetworks(arrayOf(SeeWApp.underlyingNetwork))
                    ?: setUnderlyingNetworks(arrayOf(SeeWApp.underlyingNetwork))
            }
        }
    }
