...
@RequiresApi(24)
class TileService : BaseTileService(), SeeWConnection.Callback {
    ...
    private val connection = SeeWConnection(SeeWConnection.CONNECTION_ID_TILE)
    ...
    override fun cbSelectorUpdate(id: Long) {
        val profile = SeeWDatabase.proxyDao.getById(id) ?: return
    ...
    private fun toggle() {
        ...
        if (service == null) tapPending =
            ...
            when {
                state.canStop -> SeeWApp.stopService()
                state == BaseService.State.Stopped -> SeeWApp.startService()
            }
        }
    }
}
