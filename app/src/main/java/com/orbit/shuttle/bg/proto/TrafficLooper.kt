...
import com.orbit.shuttle.fmt.TAG_BYPASS
import com.orbit.shuttle.fmt.TAG_PROXY
    ...
    private suspend fun loop() {
        ...
        while (sc.isActive) {
            ...
            if (data.state == BaseService.State.Connected
                && data.binder.callbackIdMap.containsValue(SeeWConnection.CONNECTION_ID_MAIN_ACTIVITY_FOREGROUND)
            ) {
                data.binder.broadcast { b ->
                    if (data.binder.callbackIdMap[b] == SeeWConnection.CONNECTION_ID_MAIN_ACTIVITY_FOREGROUND) {
