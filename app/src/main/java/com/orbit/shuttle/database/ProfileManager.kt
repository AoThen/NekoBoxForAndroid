...
import com.orbit.shuttle.ktx.applyDefaultValues
...
object ProfileManager {
    ...
    suspend fun createProfile(groupId: Long, bean: AbstractBean): ProxyEntity {
        ...
        val profile = ProxyEntity(groupId = groupId).apply {
            id = 0
            putBean(bean)
            userOrder = SeeWDatabase.proxyDao.nextOrder(groupId) ?: 1
        }
        profile.id = SeeWDatabase.proxyDao.addProxy(profile)
        iterator { onAdd(profile) }
        return profile
    }

    suspend fun updateProfile(profile: ProxyEntity) {
        SeeWDatabase.proxyDao.updateProxy(profile)
        iterator { onUpdated(profile, false) }
    }

    suspend fun updateProfile(profiles: List<ProxyEntity>) {
        SeeWDatabase.proxyDao.updateProxy(profiles)
        profiles.forEach {
    ...
    suspend fun deleteProfile2(groupId: Long, profileId: Long) {
        if (SeeWDatabase.proxyDao.deleteById(profileId) == 0) return
    ...
    suspend fun deleteProfile(groupId: Long, profileId: Long) {
        ...
        if (SeeWDatabase.proxyDao.countByGroup(groupId) > 1) {
    ...
    fun getProfile(profileId: Long): ProxyEntity? {
        ...
        return try {
            SeeWDatabase.proxyDao.getById(profileId)
    ...
    fun getProfiles(profileIds: List<Long>): List<ProxyEntity> {
        ...
        return try {
            SeeWDatabase.proxyDao.getEntities(profileIds)
        } catch (ex: SQLiteCantOpenDatabaseException) {
            throw IOException(ex)
        } catch (ex: SQLException) {
            Logs.w(ex)
    ...
    suspend fun createRule(rule: RuleEntity, post: Boolean = true): RuleEntity {
        rule.userOrder = SeeWDatabase.rulesDao.nextOrder() ?: 1
        rule.id = SeeWDatabase.rulesDao.createRule(rule)
        if (post) {
    ...
    suspend fun updateRule(rule: RuleEntity) {
        SeeWDatabase.rulesDao.updateRule(rule)
        ruleIterator { onUpdated(rule) }
    }

    suspend fun deleteRule(ruleId: Long) {
        SeeWDatabase.rulesDao.deleteById(ruleId)
        ruleIterator { onRemoved(ruleId) }
    }

    suspend fun deleteRules(rules: List<RuleEntity>) {
        SeeWDatabase.rulesDao.deleteRules(rules)
        ruleIterator {
            rules.forEach {
                onRemoved(it.id)
            }
    ...
    suspend fun getRules(): List<RuleEntity> {
        var rules = SeeWDatabase.rulesDao.allRules()
        if (rules.isEmpty() && !DataStore.rulesFirstCreate) {
            ...
            rules = SeeWDatabase.rulesDao.allRules()
        }
        return rules
    }
