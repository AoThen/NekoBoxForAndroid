...
import com.orbit.shuttle.CONNECTION_TEST_URL
import com.orbit.shuttle.GroupType
import com.orbit.shuttle.IPv6Mode
import com.orbit.shuttle.Key
import com.orbit.shuttle.TunImplementation
...
import com.orbit.shuttle.database.preference.PublicDatabase
import com.orbit.shuttle.database.preference.RoomPreferenceDataStore
import com.orbit.shuttle.ktx.boolean
import com.orbit.shuttle.ktx.int
import com.orbit.shuttle.ktx.long
import com.orbit.shuttle.ktx.parsePort
import com.orbit.shuttle.ktx.string
import com.orbit.shuttle.ktx.stringToInt
import com.orbit.shuttle.ktx.stringToIntIfExists
import com.orbit.shuttle.module.TempDatabase
...
object DataStore : OnPreferenceDataStoreChangeListener {
    ...
    fun currentGroupId(): Long {
        ...
        val groupId = SeeWDatabase.groupDao.createGroup(ProxyGroup(ungrouped = true))
        selectedGroup = groupId
        return groupId
    }
    ...
    fun currentGroup(): ProxyGroup {
        ...
        if (currentSelected > 0L) {
            group = SeeWDatabase.groupDao.getById(currentSelected)
        }
        if (group != null) return group
        val groups = SeeWDatabase.groupDao.allGroups()
        if (groups.isEmpty()) {
            group = ProxyGroup(ungrouped = true).apply {
                id = SeeWDatabase.groupDao.createGroup(this)
            }
        } else {
            group = groups[0]
        }
