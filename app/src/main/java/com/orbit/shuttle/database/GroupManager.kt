...
import com.orbit.shuttle.bg.SubscriptionUpdater
...
object GroupManager {
    ...
    suspend fun clearGroup(groupId: Long) {
        ...
        SeeWDatabase.proxyDao.deleteAll(groupId)
        iterator { groupUpdated(groupId) }
    }

    fun rearrange(groupId: Long) {
        val entities = SeeWDatabase.proxyDao.getByGroup(groupId)
        for (index in entities.indices) {
            entities[index].userOrder = (index + 1).toLong()
        }
        SeeWDatabase.proxyDao.updateProxy(entities)
    }
    ...
    suspend fun postUpdate(groupId: Long) {
        postUpdate(SeeWDatabase.groupDao.getById(groupId) ?: return)
    }
    ...
    suspend fun createGroup(group: ProxyGroup): ProxyGroup {
        group.userOrder = SeeWDatabase.groupDao.nextOrder() ?: 1
        group.id = SeeWDatabase.groupDao.createGroup(group.applyDefaultValues())
    ...
    suspend fun updateGroup(group: ProxyGroup) {
        SeeWDatabase.groupDao.updateGroup(group)
        iterator { groupUpdated(group) }
        if (group.type == GroupType.SUBSCRIPTION) {
            SubscriptionUpdater.reconfigureUpdater()
        }
    ...
    suspend fun deleteGroup(groupId: Long) {
        SeeWDatabase.groupDao.deleteById(groupId)
        SeeWDatabase.proxyDao.deleteByGroup(groupId)
        iterator { groupRemoved(groupId) }
        SubscriptionUpdater.reconfigureUpdater()
    }

    suspend fun deleteGroup(group: List<ProxyGroup>) {
        SeeWDatabase.groupDao.deleteGroup(group)
        SeeWDatabase.proxyDao.deleteByGroup(group.map { it.id }.toLongArray())
